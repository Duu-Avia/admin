...
export default function Home() {
  let array = [
    ...
    const Cards = array.map((SingleCard)=>
      <div key={SingleCard.id} className="CardContainer">
        <div className="NameContainer"> 
        <h1 className="FirstName">{SingleCard.firstName}</h1>
        <h1 className="LastName">{SingleCard.lastName}</h1>
        </div>
        <h2>Designer </h2>
        <div className="logo"></div>
        <div className="BarCode"></div>
        <div className="CompanyName">{SingleCard.companyName}</div>
        
     
        <div className="text">
        <p><FiPhone/> {SingleCard.phoneNumber} </p>
          <p><MdOutlineMailOutline /> {SingleCard.email}</p>  
          <p><GrLocation /> {SingleCard.address}</p>
        </div>
